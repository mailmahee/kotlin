...
public class JetSourceNavigationHelper {
    ...
    public static PsiClass getOriginalPsiClassOrCreateLightClass(@NotNull JetClassOrObject classOrObject) {
        ...
        if (LightClassUtil.belongsToKotlinBuiltIns(containingFile)) {
            Name name = classOrObject.getNameAsName();
            assert name != null : "Class from BuiltIns should have a name";
            ClassDescriptor builtInClassDescriptor = KotlinBuiltIns.getInstance().getBuiltInClassByName(name);
            JetType jetType = builtInClassDescriptor.getDefaultType();
            Type javaAnalog = KotlinToJavaTypesMap.getInstance().getJavaAnalog(jetType);
            if (javaAnalog != null) {
                if (AsmUtil.isPrimitive(javaAnalog)) {
                    jetType = TypeUtils.makeNullable(jetType);
                    javaAnalog = KotlinToJavaTypesMap.getInstance().getJavaAnalog(jetType);
                    assert javaAnalog != null : "Java analog should exists for primitive type";
                }
                if (javaAnalog.getSort() != Type.OBJECT) {
                    return null;
                }
                FqName fqName = JvmClassName.byType(javaAnalog).getFqName();
                return JavaPsiFacade.getInstance(classOrObject.getProject()).
                        findClass(fqName.getFqName(), GlobalSearchScope.allScope(classOrObject.getProject()));
            }
        }
